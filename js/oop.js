...
*/
function Hamburger(bun, garnishes, cheeses, sauces, meatPatty, pattyCount, toppings) {
    this.bun = bun;
    ...
    this.describe = function () {
        let description = `A delicious hamburger with ${this.pattyCount} ${this.meatPatty} patty(s) on a ${this.bun} bun, `;
        if (this.cheeses.length > 0) {
            description += `topped with ${this.cheeses.join(', ')} cheese, `;
        }
        if (this.garnishes.length > 0) {
            description += `and garnished with ${this.garnishes.join(', ')}, `;
        }
        if (this.sauces.length > 0) {
            description += `served with ${this.sauces.join(', ')} sauce, `;
        }
        if (this.toppings.length > 0) {
            description += `and garnished with ${this.toppings.join(', ')}.`;
        }
        return description;
    };
}
const myburger = new Hamburger('sesame', ['tamato', 'lettuce', 'redonion'], ['cheddarcheese'], ['ketchup', 'mayonise'], 'groundbeef', 1, ['spicypickles', 'hot peppers']);
